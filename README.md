# Hi there 👋

I'm Kudzayi. I find myself at the intersection of networks, embedded systems, 
and cybersecurity — passionate about securing the connected world and building 
things that bridge the physical and digital.

I'm currently pursuing my MSc in Cyber-Physical Systems (IoT) at Northeastern 
University, where I also teach IoT Fundamentals and Embedded Systems as a 
Teaching Assistant. Thanks for stopping by!

---

## 🔧 What I Work With

**IoT & Embedded**
`ESP32` `MQTT` `CoAP` `TLS/SSL` `Python` `Java` `FreeRTOS`

**Security & Networking**
`Wireshark` `IDS/IPS` `Firewalls` `VLANs` `CCNA` `Docker`

**Cloud & Virtualization**
`AWS` `CloudFormation` `Proxmox` `KVM` `Pextra` `Linux`

---

## 🚀 Featured Work

- 🔐 **SmartScan Node** — ESP32-based elder care security system with 94.7% detection accuracy. Semi-finalist at Northeastern's Husky Pitch Summit
- 🌐 **IoT Security Homelab** — Live IDS/IPS environment with containerized services and private cloud on Proxmox/KVM at [myviewsontech.com](https://myviewsontech.com)
- ☁️ **AWS CloudFormation Lab** — Provisioned S3 and EC2 infrastructure using IaC YAML stack templates
- 📡 **CDA/GDA IoT System** — End-to-end IoT pipeline from edge device (Python) to gateway (Java) with encrypted MQTT and CoAP

---

## 📬 Let's Connect

[![Blog](https://img.shields.io/badge/Blog-myviewsontech.com-blue?style=flat&logo=google-chrome)](https://myviewsontech.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat&logo=linkedin)](https://linkedin.com/in/kudzayichimbodza)
